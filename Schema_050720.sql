...
insert into investigate select distinct lhost, date('now'), '3.Network Security', 'Suspicious Internal Network Service Connections', " ", " ", "LocalAddress: " || localaddress ||  ":" || localport || " RemoteComputer: " || rhost || " RemoteAddres: " || remoteaddress || ":" || remoteport || " ProcessName: " || processname, " " from interconnects where (((lhost like '%-l-%' or lhost like '%-w-%') and (rhost like '%-l-%' or rhost like '%-w-%' or rhost like '')) and (cast(localport as int) < 20000 or cast(remoteport as int) < 20000));
...
(select distinct computername, "OS: " || os || " Missing Sophos" as detail from av where computername not in (select computername from av where type like 'sophos') union
...
select distinct computername, "OS: " || os || " Missing Crowdstrike" as detail from av where computername not in (select computername from av where type like 'crowdstrike') union 
...
create table stat as select count(distinct(computername)) as denominator from (assets);
CREATE TABLE IF NOT EXISTS CompositeScore ('Category' text,'Numerator' int,'Denominator' int,'Result' int);
insert into compositescore select category, numerator, denominator, (round(sum((numerator*1.00) /denominator),4)*100) as result  from (select '1.Data Loss' category, (select count(distinct(computername)) from investigate where risk like '1.Data Loss') numerator, (select denominator from stat) denominator, '');
insert into compositescore select category, numerator, denominator, (round(sum((numerator*1.00) /denominator),4)*100) as result  from (select '2.User Behavior' category, (select count(distinct(computername)) from investigate where risk like '2.User Behavior') numerator, (select denominator from stat) denominator, '');
insert into compositescore select category, numerator, denominator, (round(sum((numerator*1.00) /denominator),4)*100) as result  from (select '3.Network Security' category, (select count(distinct(computername)) from investigate where risk like '3.Network Security') numerator, (select denominator from stat) denominator, '');
insert into compositescore select category, numerator, denominator, (round(sum((numerator*1.00) /denominator),4)*100) as result  from (select '4.Services Configuration' category, (select count(distinct(computername)) from investigate where risk like '4.Services Configuration') numerator, (select denominator from stat) denominator, '');
insert into compositescore select category, numerator, denominator, (round(sum((numerator*1.00) /denominator),4)*100) as result  from (select '5.Build' category, (select count(distinct(computername)) from investigate where risk like '5.Build') numerator, (select denominator from stat) denominator, '');

-- Results
create table assessment ("Assessed" int, "Investigate" int, "Risk" int);
insert into assessment select (select denominator from stat), (select count(distinct(computername)) from investigate), '';
